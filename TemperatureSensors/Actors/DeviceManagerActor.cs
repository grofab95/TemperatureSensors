...
{
    class DeviceManagerActor : UntypedActor
    {
            ...
            switch (message)
            {
                case RequestTrackDevice trackMsg:
                    ...
                    else
                    {
                        ...
                        var groupActor = Context.ActorOf(DeviceGroupActor.Props(trackMsg.GroupId), $"group-{trackMsg.GroupId}");
        ...
        public static Props Props(string groupId) => Akka.Actor.Props.Create<DeviceManagerActor>();
    }
}
